...
export default function AllProduct() {
    ...
    const addToCart = (item: Product) => {
        ...
        const existingProductIndex = existingCart.findIndex((cartItem: any) => cartItem.id === item.id);

        const displayName = i18n.language === 'bn' ? item.name_bn : item.name_en;

        const cartItem = {
            id: item.id,
            name: displayName,
            image: item.image, // Use the image URL already mapped in state
            price: item.price,
            quantity: 1 // Default to 1 for "Add to Cart" from grid view
        };
