...
export function AddProductForm({ onSubmit, isLoading }: AddProductFormProps) {
  ...
  console.log('🟢 [ADD FORM] AddProductForm rendered');
  ...
  useEffect(() => {
    if (sellerPrice && sellerPrice > 0) {
      ...
      const sellingVal = Math.round(seller + (seller * 0.30)); // 1.3x cost = Selling Price
      const mrpVal = Math.round(sellingVal + (sellingVal * 0.20)); // 1.2x selling = MRP

      // Key mappings based on user's manual label changes:
  ...
  const handleAutoGenerateSKU = () => {
    if (productTitleEnglish) {
      ...
      setValue('sku', sku);
      console.log('🔧 [ADD FORM] Auto-generated SKU:', sku);
    }
  };

  // Handle main image change
  const handleMainImageChange = (file: File | null) => {
    console.log('🖼️ [ADD FORM] Main image changed:', file?.name || 'No file');
    setMainImage(file);
    if (file) {
  ...
  const handleGalleryImagesChange = (files: File[]) => {
    console.log('📸 [ADD FORM] Gallery images changed:', files.length, 'files');
  ...
  // Clear form handler
  const handleClear = () => {
    console.log('🗑️ [ADD FORM] Clear form clicked');
    setMainImage(null);
    setGalleryImages([]);
